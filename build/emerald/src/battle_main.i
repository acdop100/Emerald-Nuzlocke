...
__attribute__((section("ewram_data"))) struct UnknownPokemonStruct4 gUnknown_02022FF8[3] = {0};
__attribute__((section("ewram_data"))) struct UnknownPokemonStruct4* gUnknown_02023058 = ((void *)0);
...
static void sub_8036A5C(void)
{
    ...
    *(&gBattleStruct->field_183) = r6 >> 8;
    gBattleStruct->field_183 |= FlagGet(((0x500 + 0x356 + 0xA) + 0x72)) << 7;
}
...
static void sub_80379F8(u8 arrayIdPlus)
{
    ...
    for (i = 0; i < 3; i++)
    {
        gUnknown_02022FF8[i].species = GetMonData(&gPlayerParty[arrayIdPlus + i], 11);
        gUnknown_02022FF8[i].heldItem = GetMonData(&gPlayerParty[arrayIdPlus + i], 12);
        GetMonData(&gPlayerParty[arrayIdPlus + i], 2, gUnknown_02022FF8[i].nickname);
        gUnknown_02022FF8[i].level = GetMonData(&gPlayerParty[arrayIdPlus + i], 56);
        gUnknown_02022FF8[i].hp = GetMonData(&gPlayerParty[arrayIdPlus + i], 57);
        gUnknown_02022FF8[i].maxhp = GetMonData(&gPlayerParty[arrayIdPlus + i], 58);
        gUnknown_02022FF8[i].status = GetMonData(&gPlayerParty[arrayIdPlus + i], 55);
        gUnknown_02022FF8[i].personality = GetMonData(&gPlayerParty[arrayIdPlus + i], 0);
        ...
        if (GetMonData(&gPlayerParty[arrayIdPlus + i], 3) != 1)
            PadNameString(gUnknown_02022FF8[i].nickname, 0x00);
    }
...
static void sub_8038F34(void)
{
    ...
    case 2:
        if (!gPaletteFade.active)
        {
            ...
            {
                if (FlagGet(((0x500 + 0x356 + 0xA) + 0x72)))
                {
                    FreeAllWindowBuffers();
...
u8 IsRunningFromBattleImpossible(void)
{
    ...
    if (i != 0 && ((gBattleMons[gActiveBattler].type1 == 0x08 || gBattleMons[gActiveBattler].type2 == 0x08)))
    {
...
void sub_803BDA0(u8 battler)
{
    ...
    if (gBattleTypeFlags & 0x0001)
    {
        for (i = 0; i < 3; i++)
        {
            ...
            *(((battler) ^ 2) * 3 + i + (u8*)(gBattleStruct->field_60)) = gUnknown_0203CF00[i];
        }
    }
    else
    {
                ...
                case 2:
                    *(gBattleStruct->field_58 + gActiveBattler) = gBattlerPartyIndexes[gActiveBattler];
                    if (gBattleMons[gActiveBattler].status2 & (0x0000E000 | 0x04000000)
                        ...
                        || gStatuses3[gActiveBattler] & 0x400)
                    {
                        BtlController_EmitChoosePokemon(0, PARTY_CANT_SWITCH, 6, 0, gBattleStruct->field_60[gActiveBattler]);
                    }
                             ...
                             || ((i = (AbilityBattleEffects(0xC, gActiveBattler, 71, 0, 0)))
                                 ...
                                 && gBattleMons[gActiveBattler].ability != 26)
                             || ((i = AbilityBattleEffects(0xF, gActiveBattler, 42, 0, 0))
                                 && ((gBattleMons[gActiveBattler].type1 == 0x08 || gBattleMons[gActiveBattler].type2 == 0x08))))
                    {
                    ...
                    else
                    {
                        if (gActiveBattler == 2 && gChosenActionByBattler[0] == 2)
                            BtlController_EmitChoosePokemon(0, PARTY_CHOOSE_MON, *(gBattleStruct->monToSwitchIntoId + 0), 0, gBattleStruct->field_60[gActiveBattler]);
                        else if (gActiveBattler == 3 && gChosenActionByBattler[1] == 2)
                            BtlController_EmitChoosePokemon(0, PARTY_CHOOSE_MON, *(gBattleStruct->monToSwitchIntoId + 1), 0, gBattleStruct->field_60[gActiveBattler]);
                        else
                            BtlController_EmitChoosePokemon(0, PARTY_CHOOSE_MON, 6, 0, gBattleStruct->field_60[gActiveBattler]);
                    }
                    MarkBattlerForControllerExec(gActiveBattler);
                    break;
                case 5:
...
static void HandleAction_UseMove(void)
{
        ...
        && gBattleMons[gSideTimers[side].followmeTarget].hp != 0)
    {
        gBattlerTarget = gSideTimers[side].followmeTarget;
             ...
             && gBattleMoves[gCurrentMove].type == 0x0d)
    {
