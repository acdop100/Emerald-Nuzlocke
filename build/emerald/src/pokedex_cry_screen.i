...
static __attribute__((section("ewram_data"))) struct PokedexCryVolumeMeter *sCryVolumeMeter = ((void *)0);
...
bool8 sub_8145354(struct CryRelatedStruct *arg0, u8 windowId)
{
    ...
    case 0:
        ...
        for (i = 0; i < 224; i++)
            CopyToWindowPixelBuffer(windowId, gUnknown_085B8C10, 32, i);
...
{
    if (gMPlayInfo_BGM.status & 0x80000000 && !sDexCryScreen->unk1A)
    {
        if (!sDexCryScreen->unk1B)
        {
...
static void sub_8145648(u8 windowId)
{
    ...
    for (i = 0; i < 7; i++)
        CopyToWindowPixelBuffer(windowId, gUnknown_085B8C10, 32, offset + (i * 32));
}
...
static void sub_81456A8(u8 a0, u8 a1)
{
    ...
    if (i > sDexCryScreen->unk12)
    {
        do
        {
            offset = gUnknown_085B8770[a0 & 0x7][i] + (a0 / 8) * 32;
...
void sub_8145914(void)
{
    ...
    { Free(sCryVolumeMeter); sCryVolumeMeter = ((void *)0); };
}
...
static void sub_814596C(struct Sprite *sprite)
{
    ...
    gSprites[sCryVolumeMeter->needleSpriteId].oam.affineMode = 1;
