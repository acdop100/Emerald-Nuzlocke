...
# 3 "src/trader.c" 2
# 1 "include/constants/mauville_man.h" 1
# 4 "src/trader.c" 2
...
void CreateAvailableDecorationsMenu(u8 taskId)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (trader->decorIds[i] > 120)
            ...
            AddTextPrinterParameterized(data[3], 1, gDecorations[trader->decorIds[i]].name, 8, 16 * i + 1, 255, ((void *)0));
    }
