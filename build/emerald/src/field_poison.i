...
{
    struct Pokemon *pokemon = gPlayerParty + partyIdx;
    if (IsMonValidSpecies(pokemon) && GetMonData(pokemon, 57) == 0 && pokemon_ailments_get_primary(GetMonData(pokemon, 55)) == AILMENT_PSN)
    {
        return 1;
    }
    return 0;
...
s32 DoPoisonFieldEffect(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (GetMonData(pokemon, 5) && pokemon_ailments_get_primary(GetMonData(pokemon, 55)) == AILMENT_PSN)
        {
