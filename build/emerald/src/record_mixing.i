...
struct PlayerRecordsEmerald
{
                 ...
                 u16 giftItem;
                 LilycoveLady lilycoveLady;
                 struct Apprentice apprentice[2];
...
static void ReceiveGiftItem(u16 *item, u8 which)
{
    if (which != 0 && *item != 0 && GetPocketByItemId(*item) == POCKET_KEY_ITEMS)
    {
...
static void ReceiveApprenticeData(struct Apprentice *arg0, size_t arg1, u32 arg2)
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (structPtr[i].playerName[0] != 0xFF && !sub_80E841C(&structPtr[i], &gSaveBlock2Ptr->apprentices[0]))
        {
            r7++;
            r8 = i;
        }
