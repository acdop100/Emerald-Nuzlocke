...
const u8 *GetInteractedLinkPlayerScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (gLinkPlayerEventObjects[i].active == 1 && gLinkPlayerEventObjects[i].eventObjId == eventObjectId)
            return ((void *)0);
    }
...
static const u8 *GetInteractedMetatileScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
    ...
    if (MetatileBehavior_IsUnknownClosedDoor(metatileBehavior) == 1)
...
{
    if (FlagGet(((0x500 + 0x356 + 0xA) + 0xB)) == 1 && PartyHasMonWithSurf() == 1 && IsPlayerFacingSurfableFishableWater() == 1)
        return EventScript_UseSurf;

    if (MetatileBehavior_IsWaterfall(metatileBehavior) == 1)
    {
        if (FlagGet(((0x500 + 0x356 + 0xA) + 0xE)) == 1 && IsPlayerSurfingNorth() == 1)
...
static bool32 TrySetupDiveDownScript(void)
{
    if (FlagGet(((0x500 + 0x356 + 0xA) + 0xD)) && TrySetDiveWarp() == 2)
    {
...
static bool32 TrySetupDiveEmergeScript(void)
{
    if (FlagGet(((0x500 + 0x356 + 0xA) + 0xD)) && gMapHeader.mapType == 5 && TrySetDiveWarp() == 1)
    {
