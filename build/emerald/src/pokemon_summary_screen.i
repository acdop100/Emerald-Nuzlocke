...
# 1 "include/text.h" 1
# 41 "src/pokemon_summary_screen.c" 2
# 1 "include/tv.h" 1
...
# 1 "include/window.h" 1
# 43 "src/pokemon_summary_screen.c" 2
...
# 101 "src/pokemon_summary_screen.c"
...
static __attribute__((section("ewram_data"))) u8 sUnknown_0203CF21 = 0;
...
# 300 "src/pokemon_summary_screen.c" 2
...
# 301 "src/pokemon_summary_screen.c" 2

__attribute__((aligned(4))) static const struct BgTemplate sUnknown_0861CBB4[] =
{
    {
        .bg = 0,
        .charBaseIndex = 0,
...
static const u8 sMoveTypeToOamPaletteNum[0x12 + 5] =
{
    [0x00] = 13,
    [0x01] = 13,
    [0x02] = 14,
...
static void SummaryScreen_LoadingCB2(void)
{
    while (sub_81221EC() != 1 && SummaryScreen_LoadGraphics() != 1 && sub_81221AC() != 1);
}
...
static bool8 SummaryScreen_LoadGraphics(void)
{
    ...
    case 5:
        ...
        break;
    case 6:
        if (SummaryScreen_DecompressGraphics() != 0)
            gMain.state++;
        break;
    case 7:
        ResetWindows();
...
static void InitBGs(void)
{
    ...
    InitBgsFromTemplates(0, sUnknown_0861CBB4, (size_t)(sizeof(sUnknown_0861CBB4) / sizeof((sUnknown_0861CBB4)[0])));
...
static bool8 ExtractMonDataToSummaryStruct(struct Pokemon *mon)
{
    ...
    case 3:
        ...
        ConvertInternationalString((u8*)&sum->OTName, GetMonData(mon, 3));
...
static void HandleInput(u8 taskId)
{
    if (sub_81221EC() != 1 && !gPaletteFade.active)
    {
        ...
        else if ((gMain.newKeys & 0x0020) || GetLRKeysState() == 1)
        {
            ChangePage(taskId, -1);
        }
        else if ((gMain.newKeys & 0x0010) || GetLRKeysState() == 2)
        {
            ChangePage(taskId, 1);
        }
    ...
    else
    {
        ...
        do
        {
            ...
            if (index < 0 || index > sMonSummaryScreen->maxMonIndex)
                return -1;
        } while (GetMonData(&mon[index], 45) != 0);
        return index;
    }
}
...
static bool8 sub_81C0A50(struct Pokemon* mon)
{
    if (GetMonData(mon, 11) == 0)
        return 0;
    else if (sMonSummaryScreen->curMonIndex != 0 || GetMonData(mon, 45) == 0)
        return 1;
    else
        return 0;
}
...
static void HandleInput_MoveSelect(u8 taskId)
{
    ...
    if (sub_81221EC() != 1)
    {
        ...
        {
            if (sMonSummaryScreen->unk40C8 == 1
...
static void sub_81C1070(s16 *a, s8 b, u8 *c)
{
    ...
    schedule_bg_copy_tilemap_to_vram(2);
    PrintMoveDetails(move);
    if ((*c == 4 && sMonSummaryScreen->newMove == 0) || a[1] == 1)
    {
        ClearWindowTilemap(19);
    ...
    if (*c != 4 && moveIndex == 4 && sMonSummaryScreen->newMove == 0)
    {
...
static bool8 CanReplaceMove(void)
{
    if (sMonSummaryScreen->firstMoveIndex == 4 || sMonSummaryScreen->newMove == 0 || IsMoveHm(sMonSummaryScreen->summary.moves[sMonSummaryScreen->firstMoveIndex]) != 1)
        return 1;
    else
        return 0;
}
...
static void sub_81C1CB0(const struct UnkStruct_61CC04 *unkStruct, u16 *dest, u8 c, bool8 d)
{
    ...
    if (unkStruct->field_6 != c)
    {
        if (!d)
        {
            ...
            {
                CpuSet(&unkStruct->ptr[c + unkStruct->field_6 * i], &alloced[unkStruct->field_6 * i], 0x00000000 | (((unkStruct->field_6 - c) * 2)/(16/8) & 0x1FFFFF));
            }
        }
        else
        {
            for (i = 0; i < unkStruct->field_7; i++)
            {
                CpuSet(&unkStruct->ptr[unkStruct->field_6 * i], &alloced[c + unkStruct->field_6 * i], 0x00000000 | (((unkStruct->field_6 - c) * 2)/(16/8) & 0x1FFFFF));
            }
        }
    }
    for (i = 0; i < unkStruct->field_7; i++)
...
static void PrintHeldItemName(void)
{
    ...
    if (sMonSummaryScreen->summary.item == 175 && IsMultiBattle() == 1 && (sMonSummaryScreen->curMonIndex == 1 || sMonSummaryScreen->curMonIndex == 4 || sMonSummaryScreen->curMonIndex == 5))
    {
...
static void sub_81C40A0(u8 moveIndex1, u8 moveIndex2)
{
    ...
    FillWindowPixelRect(windowId1, ((0) | ((0) << 4)), 0, moveIndex1 * 16, 0x48, 0x10);
    FillWindowPixelRect(windowId1, ((0) | ((0) << 4)), 0, moveIndex2 * 16, 0x48, 0x10);

    FillWindowPixelRect(windowId2, ((0) | ((0) << 4)), 0, moveIndex1 * 16, 0x30, 0x10);
    FillWindowPixelRect(windowId2, ((0) | ((0) << 4)), 0, moveIndex2 * 16, 0x30, 0x10);
...
static void HidePageSpecificSprites(void)
{
    ...
    for (i = 3; i < (size_t)(sizeof(sMonSummaryScreen->spriteIds) / sizeof((sMonSummaryScreen->spriteIds)[0])); i++)
    {
...
static void SetMonTypeIcons(void)
{
    ...
    if (summary->isEgg)
    {
        SetMoveTypeSpritePosAndType(0x09, 120, 48, 3);
...
static void SetContestMoveTypeIcons(void)
{
    ...
    {
        if (summary->moves[i] != 0)
            SetMoveTypeSpritePosAndType(0x12 + gContestMoves[summary->moves[i]].contestCategory, 0x55, 0x20 + (i * 0x10), i + 3);
        else
            SetSpriteInvisibility(i + 3, 1);
    }
}
...
static void SetNewMoveTypeIcon(void)
{
    ...
    else
    {
        if (sMonSummaryScreen->currPageIndex == 2)
            ...
            SetMoveTypeSpritePosAndType(0x12 + gContestMoves[sMonSummaryScreen->newMove].contestCategory, 85, 96, 7);
    }
}
...
static void StopPokemonAnimations(void)
{
    ...
    gSprites[sMonSummaryScreen->spriteIds[0]].animPaused = 1;
