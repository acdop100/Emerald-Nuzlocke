...
extern const u8 gReelSymbols[][21];
...
static void SlotMachineSetup_0_1(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        sSlotMachine->reelTagOffsets[i] = gInitialReelPositions[i][sSlotMachine->luckyGame] % 21;
        sSlotMachine->reelPixelOffsets[i] = 21 * 24 - sSlotMachine->reelTagOffsets[i] * 24;
...
{
    s16 tagIndex = (sSlotMachine->reelTagOffsets[reelIndex] + posOffset) % 21;
    if (tagIndex < 0)
        tagIndex += 21;
...
static void AdvanceSlotReel(u8 reelIndex, s16 value)
{
    ...
    sSlotMachine->reelTagOffsets[reelIndex] = 21 - sSlotMachine->reelPixelOffsets[reelIndex] / 24;
}
...
static void DecideReelTurns_NoBiasTag_Reel3_Bet2(void)
{
    ...
    while (1)
    {
        ...
        {
            tag1 = GetNearbyTag(0, i - sSlotMachine->reelExtraTurns[0]);
