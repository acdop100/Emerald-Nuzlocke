...
__attribute__((section("ewram_data"))) static void(*gUnknown_0203A0F4)(u8 taskId) = ((void *)0);
...
static const u8 gUnknown_085920E4[] = {2, 4, 1, 3};
...
void ItemUseOutOfBattle_Bike(u8 taskId)
{
    ...
    if (FlagGet(((0x500 + 0x356 + 0xA) + 0x2B)) == 1 || MetatileBehavior_IsVerticalRail(behavior) == 1 || MetatileBehavior_IsHorizontalRail(behavior) == 1 || MetatileBehavior_IsIsolatedVerticalRail(behavior) == 1 || MetatileBehavior_IsIsolatedHorizontalRail(behavior) == 1)
    ...
    for (curX = x - 7; curX <= x + 7; curX++)
    {
        for (curY = y - 5; curY <= y + 5; curY++)
        {
             ...
             || curY >= height)
            {
                ...
                if (conn && sub_80FD730(conn, curX, curY) == 1)
...
void sub_80FDA94(u8 taskId)
{
    if (EventObjectCheckHeldMovementStatus(&gEventObjects[GetEventObjectIdByLocalIdAndMap(0xFF, 0, 0)]) == 1)
...
void ItemUseOutOfBattle_TMHM(u8 taskId)
{
    if (gSpecialVar_ItemId >= 339)
        DisplayItemMessage(taskId, 1, gText_BootedUpHM, sub_80FDF90);
    else
        DisplayItemMessage(taskId, 1, gText_BootedUpTM, sub_80FDF90);
}
