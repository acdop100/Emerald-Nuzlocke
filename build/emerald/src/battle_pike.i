...
static bool8 DoesTypePreventStatus(u16 species, u32 status)
{
    ...
    {
    case 0x80:
        if (gBaseStats[species].type1 == 0x08 || gBaseStats[species].type1 == 0x03
            || gBaseStats[species].type2 == 0x08 || gBaseStats[species].type2 == 0x03)
            ret = 1;
        break;
    case 0x20:
        if (gBaseStats[species].type1 == 0x0f || gBaseStats[species].type2 == 0x0f)
            ret = 1;
        break;
    case 0x40:
        if (gBaseStats[species].type1 == 0x04 || gBaseStats[species].type1 == 0x0d
            || gBaseStats[species].type2 == 0x04 || gBaseStats[species].type2 == 0x0d)
            ret = 1;
        break;
    case 0x10:
        if (gBaseStats[species].type1 == 0x0a || gBaseStats[species].type2 == 0x0a)
            ret = 1;
        break;
    case 0x7:
        break;
    ...
    do
    {
        ...
        if (status != sStatusFlags)
        {
            ...
            for (i = 0; i < 3; i++)
            {
                ...
                if (pokemon_ailments_get_primary(GetMonData(mon, 55)) == 0
                    && GetMonData(mon, 57) != 0)
                {
                    j++;
...
static u8 GetNextRoomType(void)
{
    ...
    if (roomTypesAvailability[7] != 1 && !AtLeastTwoAliveMons())
    {
        roomTypesAvailability[7] = 1;
        numRoomCandidates--;
    }
    if (roomTypesAvailability[3] != 1 && !AtLeastOneHealthyMon())
    {
        roomTypesAvailability[3] = 1;
        numRoomCandidates--;
    }
    ...
    {
        if (roomTypesAvailability[1] != 1)
        {
            roomTypesAvailability[1] = 1;
            numRoomCandidates--;
        }
        if (roomTypesAvailability[4] != 1)
        {
            roomTypesAvailability[4] = 1;
            numRoomCandidates--;
        }
    }
...
static void TryHealMons(u8 healCount)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        else if (pokemon_ailments_get_primary(GetMonData(mon, 55)) != 0)
        {
            canBeHealed = 1;
        }
        else
...
static void CanAnyPartyMonsBeHealed(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        u16 curr = GetMonData(mon, 57);
        u16 max = GetMonData(mon, 58);
        if (curr >= max && pokemon_ailments_get_primary(GetMonData(mon, 55)) == 0)
        {
            u8 ppBonuses = GetMonData(mon, 21);
            for (j = 0; j < 4; j++)
            {
