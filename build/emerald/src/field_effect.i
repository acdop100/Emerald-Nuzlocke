...
static void mapldr_08084390(void)
{
    ...
    {
        EventObjectTurn(&gEventObjects[gPlayerAvatar.eventObjectId], 3);
    }
    ScriptContext2_Enable();
    FreezeEventObjects();
    gFieldCallback = ((void *)0);
...
static void sub_80B6B94(u8 taskId)
{
    struct Task *task;
    task = &gTasks[taskId];
    while (gUnknown_0855C3C8[task->data[0]](task));
}

static bool8 sub_80B6BCC(struct Task *task)
{
...
static bool8 sub_80B6C74(struct Task *task)
{
    if (IsWeatherNotFadingIn())
    {
        task->data[0]++;
...
static bool8 sub_80B6C90(struct Task *task)
{
    struct Sprite *sprite;
...
static bool8 sub_80B7114(struct Task *task)
{
    ...
    CameraObjectReset2();
    eventObject = &gEventObjects[gPlayerAvatar.eventObjectId];
    EventObjectSetHeldMovement(eventObject, GetFaceDirectionMovementAction(4));
...
static bool8 sub_80B72F4(struct Task *task)
{
    ...
    if (EventObjectClearHeldMovementIfFinished(eventObject))
    {
        CameraObjectReset1();
        ScriptContext2_Disable();
        EventObjectSetHeldMovement(eventObject, GetWalkNormalMovementAction(4));
...
{
    EventObjectSetHeldMovement(eventObject, GetWalkSlowMovementAction(2));
    task->data[0]++;
    return 0;
}
...
static bool8 sub_80B7968(struct Task *task, struct EventObject *eventObject, struct Sprite *sprite)
{
    ...
    if (sprite->animCmdIndex > 1)
    {
        ...
        CameraObjectReset1();
        PlaySE(175);
        EventObjectSetHeldMovement(eventObject, GetJumpMovementAction(4));
    }
    return 0;
}
...
static void sub_80B8EA8(struct Task *task)
{
    ...
    if (!FieldEffectActiveListContains(6))
    {
        ...
        eventObject->fieldEffectSpriteId = FieldEffectStart(8);
        task->data[0]++;
    }
}
...
static void sub_80B92F8(struct Task *task)
{
    if (sub_80B9508(task->data[1]))
    {
        ...
        EventObjectSetHeldMovement(&gEventObjects[gPlayerAvatar.eventObjectId], 0x2);
    }
}
...
void sub_80B9C54(s16* data, u8 taskId)
{
    if (++data[3] > 0x78)
    {
        ...
        gEventObjects[data[2]].invisible = 1;
