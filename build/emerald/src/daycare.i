...
static s32 GetSlotToInheritNature(struct DayCare *daycare)
{
    ...
    if (GetBoxMonData(&daycare->mons[slot].mon, 12) != 195
        || Random() >= 65535 / 2)
    {
        return -1;
    }
...
static void _TriggerPendingDaycareEgg(struct DayCare *daycare)
{
    ...
    {
        u8 wantedNature = GetNatureFromPersonality(GetBoxMonData(&daycare->mons[natureSlot].mon, 0, ((void *)0)));
        u32 personality;
...
static void InheritIVs(struct Pokemon *egg, struct DayCare *daycare)
{
    ...
    u8 selectedIvs[3];
    u8 availableIVs[6];
    u8 whichParent[(size_t)(sizeof(selectedIvs) / sizeof((selectedIvs)[0]))];
    u8 iv;
    ...
    for (i = 0; i < (size_t)(sizeof(selectedIvs) / sizeof((selectedIvs)[0])); i++)
    {
...
static bool8 _DoEggActions_CheckHatch(struct DayCare *daycare)
{
    ...
    if (daycare->offspringPersonality == 0 && validEggs == 2 && (daycare->mons[1].steps & 0xFF) == 0xFF)
    {
        ...
        if (loveScore > (Random() * 100u) / 65535)
