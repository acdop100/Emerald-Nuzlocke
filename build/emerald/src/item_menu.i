...
static __attribute__((section("ewram_data"))) struct TempWallyStruct *gUnknown_0203CE80 = 0;
...
void sub_81AAC28(void)
{
    GoToBagMenu(9, POCKETS_COUNT, bag_menu_leave_maybe);
...
void FavorLadyOpenBagMenu(void)
{
    GoToBagMenu(7, POCKETS_COUNT, CB2_FavorLadyExitBagMenu);
    gSpecialVar_Result = 0;
}

void QuizLadyOpenBagMenu(void)
{
    GoToBagMenu(8, POCKETS_COUNT, CB2_QuizLadyExitBagMenu);
    gSpecialVar_Result = 0;
}
...
void GetItemName(s8 *dest, u16 itemId)
{
    switch (gBagPositionStruct.pocket)
    {
        case TMHM_POCKET:
            ...
            if (itemId >= 339)
            {
                ConvertIntToDecimalStringN(gStringVar1, itemId - 339 + 1, 2, 1);
                StringExpandPlaceholders(dest, gText_ClearTo11Var1Clear5Var2);
            }
            else
            {
                ConvertIntToDecimalStringN(gStringVar1, itemId - 289 + 1, 2, 2);
                StringExpandPlaceholders(dest, gText_UnkF908Var1Clear7Var2);
            }
            break;
        case BERRIES_POCKET:
            ConvertIntToDecimalStringN(gStringVar1, itemId - 133 + 1, 2, 2);
...
void PrintItemQuantityPlusGFX(u8 rboxId, s32 item_index_in_pocket, u8 a)
{
    ...
    if (item_index_in_pocket != -2)
    {
        ...
        else if (gBagPositionStruct.pocket != KEYITEMS_POCKET && (unique = ItemId_GetImportance(itemId)) == 0)
        {
...
bool8 UseRegisteredKeyItemOnField(void)
{
    ...
    if (InUnionRoom() == 1 || InBattlePyramid() || InBattlePike() || InMultiBattleRoom() == 1)
        return 0;
