...
# 1 "include/window.h" 1
# 31 "src/battle_controller_player.c" 2
# 1 "include/constants/battle_anim.h" 1
...
static void HandleInputChooseMove(void)
{
    ...
    if (gMain.newKeys & 0x0001)
    {
        ...
        {
            if (moveInfo->monType1 != 0x07 && moveInfo->monType2 != 0x07)
                moveTarget = 0x10;
            else
                moveTarget = 0x0;
        }
...
static void PlayerHandleChoosePokemon(void)
{
    ...
    if (gBattleTypeFlags & 0x40000 && (gBattleBufferA[gActiveBattler][1] & 0xF) != PARTY_CANT_SWITCH)
    {
