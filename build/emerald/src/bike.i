...
bool8 player_should_look_direction_be_enforced_upon_movement(void)
{
    if (TestPlayerAvatarFlags((1 << PLAYER_AVATAR_STATE_ACRO_BIKE)) != 0 && MetatileBehavior_IsBumpySlope(gEventObjects[gPlayerAvatar.eventObjectId].currentMetatileBehavior) != 0)
        return 0;
    else
        return 1;
}
...
bool32 IsRunningDisallowed(u8 metatile)
{
    if (!(gMapHeader.flags & 4) || IsRunningDisallowedByMetatile(metatile) == 1)
        return 1;
    else
        return 0;
}
