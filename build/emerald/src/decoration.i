...
__attribute__((section("ewram_data"))) struct DecorationPCContext gDecorationContext = {};
__attribute__((section("ewram_data"))) static u8 sDecorMenuWindowIds[4] = {};
__attribute__((section("ewram_data"))) static struct DecorationItemsMenu *sDecorationItemsMenu = ((void *)0);
__attribute__((section("ewram_data"))) struct PlaceDecorationGraphicsDataBuffer sPlaceDecorationGraphicsDataBuffer = {};
...
void InitDecorationContextItems(void)
{
    ...
    if (gDecorationContext.isPlayerRoom == 0)
    {
    ...
    if (gDecorationContext.isPlayerRoom == 1)
    {
...
void DoSecretBaseDecorationMenu(u8 taskId)
{
    ...
    gDecorationContext.isPlayerRoom = 0;
...
void DoPlayerRoomDecorationMenu(u8 taskId)
{
    ...
    gDecorationContext.isPlayerRoom = 1;
    gTasks[taskId].func = HandleDecorationActionsMenuInput;
}
...
{
    FillWindowPixelBuffer(sDecorMenuWindowIds[1], ((1) | ((1) << 4)));
    PrintDecorationCategoryMenuItems(taskId);
...
static void PrintDecorationItemMenuItems(u8 taskId)
{
    ...
    if ((sCurDecorationCategory < DECORCAT_DOLL || sCurDecorationCategory > DECORCAT_CUSHION) && gDecorationContext.isPlayerRoom == 1 && data[11] == 0)
...
void sub_81274A0(u8 a0, s32 a1, u8 a2)
{
    if (a1 != -2)
    {
        if (sub_81277BC(a1 + 1) == 1)
        {
            blit_move_info_icon(a0, 0x18, 0x5c, a2 + 2);
        }
        else if (sub_81277E8(a1 + 1) == 1)
        {
...
static void IdentifyOwnedDecorationsCurrentlyInUseInternal(u8 taskId)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sPlayerRoomItemsIndicesBuffer) / sizeof((sPlayerRoomItemsIndicesBuffer)[0])); i++)
    {
        if (gSaveBlock1Ptr->playerRoomDecor[i] != 0)
        {
            for (j = 0; j < gDecorationInventories[sCurDecorationCategory].size; j++)
            {
                if (gCurDecorationItems[j] == gSaveBlock1Ptr->playerRoomDecor[i] && sub_81277BC(j + 1) != 1)
                {
...
void sub_8127E18(void)
{
    ...
    for (i = 0; i < 14; i++)
    {
        if (FlagGet(0xAE + i) == 1)
        {
            ...
            VarSet(
                0x4010 + (gMapHeader.events->eventObjects[j].graphicsId - 240),
...
void sub_8127F68(u8 taskId)
{
    if (gDecorationContext.isPlayerRoom == 1 && sCurDecorationCategory != DECORCAT_DOLL && sCurDecorationCategory != DECORCAT_CUSHION)
    {
...
void SetUpPlacingDecorationPlayerAvatar(u8 taskId, struct PlaceDecorationGraphicsDataBuffer *data)
{
    ...
    if (gSaveBlock2Ptr->playerGender == 0)
        sDecor_CameraSpriteObjectIdx2 = AddPseudoEventObject(193, SpriteCallbackDummy, x, 72, 0);
    else
        sDecor_CameraSpriteObjectIdx2 = AddPseudoEventObject(194, SpriteCallbackDummy, x, 72, 0);
...
{
    if (x == gTasks[taskId].data[3] + 7 && y == gTasks[taskId].data[4] + 7 && decor != 0)
    {
        return 0;
    }
    return 1;
...
void sub_8128950(u8 taskId)
{
    if (sub_812853C(taskId, &gDecorations[gCurDecorationItems[gCurDecorationIndex]]) == 1)
    {
...
{
    CpuFastSet(&((u16 *)gTilesetPointer_SecretBase->palettes)[pal << 4], dest, ((32)/(32/8) & 0x1FFFFF));
}
...
void sub_8129088(u8 *dest, u16 tile)
{
    ...
    CpuFastSet(&((u8 *)gTilesetPointer_SecretBase->tiles)[tile << 5], buffer, ((32)/(32/8) & 0x1FFFFF));
    switch (mode)
    {
    case 0:
...
u8 AddDecorationIconObject(u8 decor, s16 x, s16 y, u8 priority, u16 tilesTag, u16 paletteTag)
{
    ...
    else if (gUnknown_085A6BE8[decor][0] == ((void *)0))
    {
...
void SetUpPuttingAwayDecorationPlayerAvatar(void)
{
    ...
    if (gSaveBlock2Ptr->playerGender == 0)
        sDecor_CameraSpriteObjectIdx2 = AddPseudoEventObject(193, SpriteCallbackDummy, 136, 72, 0);
    else
        sDecor_CameraSpriteObjectIdx2 = AddPseudoEventObject(194, SpriteCallbackDummy, 136, 72, 0);
...
bool8 sub_8129E74(u8 taskId, u8 idx, struct DecorRearrangementDataBuffer *data)
{
    ...
    ht = data->height;
    if (gDecorationContext.items[idx] == 41 && MapGridGetMetatileIdAt(xOff + 7, yOff + 7) == 0x28C)
        ht--;
    ...
    for (i = 0; i < gDecorationContext.size; i++)
    {
        if (gDecorationContext.items[i] != 0)
        {
            if (gDecorations[gDecorationContext.items[i]].permission == DECORPERM_SPRITE)
            {
                ...
                if (sub_8129E74(taskId, i, sDecorRearrangementDataBuffer) == 1)
                {
    ...
    if (sub_8129FC8(taskId) != 1)
    {
        for (i = 0; i < gDecorationContext.size; i++)
        {
            ...
            if (var1 != 0)
            {
                ...
                if (sub_8129E74(taskId, i, &sDecorRearrangementDataBuffer[0]) == 1)
                {
