...
static bool8 TryGenerateWildMon(const struct WildPokemonInfo *wildMonInfo, u8 area, u8 flags)
{
    ...
    {
    case WILD_AREA_LAND:
        if (TryGetAbilityInfluencedWildMonIndex(wildMonInfo->wildPokemon, 0x08, 42, &wildMonIndex))
            break;
        if (TryGetAbilityInfluencedWildMonIndex(wildMonInfo->wildPokemon, 0x0d, 9, &wildMonIndex))
            break;
...
static void ApplyFluteEncounterRateMod(u32 *encRate)
{
    if (FlagGet(((0x500 + 0x356 + 0xA) + 0x4D)) == 1)
        *encRate += *encRate / 2;
    else if (FlagGet(((0x500 + 0x356 + 0xA) + 0x4E)) == 1)
        *encRate = *encRate / 2;
}
